...
class HandController:
    ...
    def loop(self):
        global close_mouse_camera
        while True:
            if close_mouse_camera:
                close_mouse_camera=False
                break

            self.now = monotonic()
            frame, hands, bags = self.tracker.next_frame()
            if frame is None:
                break
            self.frame_nb += 1
            ...
            if self.use_renderer:
                frame = self.renderer.draw(frame, hands, bags)
