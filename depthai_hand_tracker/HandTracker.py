...
class HandTracker:
    ...
    def create_pipeline(self):
        ...
        if self.use_lm:
            print(f"Creating Hand Landmark Neural Network ({'2 thread' if self.lm_nb_threads == 1 else '2 threads'})...")
    ...
    def spatial_loc_roi_from_wrist_landmark(self, hand):
        ...
        rect_center = dai.Point2f(*(hand.landmarks[5]-np.array((zone_size//2 - self.crop_w, zone_size//2 + self.pad_h))))
        rect_size = dai.Size2f(zone_size, zone_size)
        return dai.Rect(rect_center, rect_size)
